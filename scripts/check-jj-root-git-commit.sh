...
[FAIL] ALLOW_GIT_COMMIT_IN_JJ_ROOT=1 예외 모드에서도 working-copy(@)가 non-empty면 git commit을 차단합니다.
       ...
       - 점검: jj st
       - 정리: scripts/finalize-and-push.sh 또는 describe_with_attribution.sh, 필요 없으면 jj abandon <change-id>
...
[FAIL] root 저장소는 jj 우선 정책입니다. git commit을 차단합니다.
       - 권장: scripts/finalize-and-push.sh 또는 describe_with_attribution.sh
