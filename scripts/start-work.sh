...
- 구현 전 배경과 목표의 초기 문구를 실제 문제와 완료 후 관찰 가능한 결과로 구체화한다.
- 영향을 받는 사용자·시스템·운영 경로, 유지할 제약과 이번 작업의 제외 범위를 기록한다.
- 이 안내는 기존 스키마를 보완한다. 초기 게이트 통과만으로 실제 요구사항 확정이나 구현 승인이 성립하지 않는다.
