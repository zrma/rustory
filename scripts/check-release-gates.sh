...
usage() {
  cat <<'USAGE'
Run consolidated release/share gates.

Usage:
  scripts/check-release-gates.sh [options]

Options:
  --work-id <id>                validate docs/todo-<work-id> readiness first
  ...
  --repo-key <key>              repository key for manifest checks (default: rustory)
...
if [[ "$MANIFEST_MODE" != "quick" && "$MANIFEST_MODE" != "full" ]]; then
  echo "invalid --manifest-mode: $MANIFEST_MODE (expected quick|full)" >&2
  exit 1
fi
...
if [[ -n "$WORK_ID" ]]; then
  ...
  warn "run without --work-id due to --allow-missing-work-id override"
fi
...
if (( fail_count > 0 )); then
  echo "[FAIL] release gates failed: $fail_count issue(s)" >&2
  exit 1
fi
...
if [[ -n "$WORK_ID" ]]; then
  if [[ -d "$todo_abs" ]]; then
    ...
    warn "skip todo readiness due --allow-missing-work-id override: $WORK_ID"
  else
    fail "$todo_rel missing while todo readiness is required"
  fi
else
  readarray -t TODO_DIRS < <(todo_workspace_find_dirs "$ROOT")
  if [[ "${#TODO_DIRS[@]}" -eq 0 ]]; then
    if [[ "$ALLOW_MISSING_WORK_ID" -eq 1 ]]; then
      warn "no todo directory found. skip todo readiness due --allow-missing-work-id override"
    else
      fail "no todo directory found. release gates require todo readiness checks unless --allow-missing-work-id is used."
    fi
  else
...
safe_run "scripts/check-prod-preflight.sh"
...
# Keep CI parity with local pre-push hook gates.
push_gate_cmd="scripts/check-push-gates.sh --mode strict"
if [[ -n "$WORK_ID" ]]; then
  printf -v push_gate_cmd '%s --work-id %q' "$push_gate_cmd" "$WORK_ID"
elif [[ "$ALLOW_MISSING_WORK_ID" -eq 1 ]]; then
  push_gate_cmd="$push_gate_cmd --allow-missing-work-id"
fi
safe_run "$push_gate_cmd"

printf -v manifest_cmd 'scripts/run-manifest-checks.sh --mode %q --repo-key %q' "$MANIFEST_MODE" "$REPO_KEY"
if [[ -n "$WORK_ID" ]]; then
  printf -v manifest_cmd '%s --work-id %q' "$manifest_cmd" "$WORK_ID"
fi
if [[ "$CLOSED_WORK_ID" -eq 1 || "$ALLOW_MISSING_WORK_ID" -eq 1 ]]; then
  manifest_cmd="$manifest_cmd --allow-missing-work-id"
fi
safe_run "$manifest_cmd"
