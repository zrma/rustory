...
- 공통 하네스와 Rustory overlay: `docs/agent-harness.md`.
- 제품/운영 탐색: `docs/HANDOFF.md`; 명령 선언: `docs/REPO_MANIFEST.yaml`.
- 실행과 출고 경계: `docs/EXECUTION_LOOP.md`, `docs/CHANGE_CONTROL.md`.
...
- Change review: 변경 전 의도·제약·acceptance, 이후 spec 변경과 이유를 diff 및
  ...
  주장하지 않는다. 검토 snapshot과 범위를 고정하고 중요한 요구사항·영향 경로별
  근거와 미확인 범위, finding의 누적 상태를 남긴다. 후발 지적은 새 회귀·이전 누락·
  새 범위·판정 불명을 구분하며, 연속 무지적을 완전성의 증거로 삼지 않는다.
  작은 변경에 별도 장부·고정 검토 횟수를 강제하지 않으며 리뷰 결과는 publication이나
  인간 판단의 권한을 부여하지 않는다.
