...
- 완료: C1, C2, v1.0.30 릴리즈, 로컬 맥북 + k8s 5개 노드 배포.
- 미완료: C3. 외부망 Linux 컨테이너에서 실제 retryable timeout 로그 샘플 확인은 해당 환경에서 수행해야 한다.
- 다음 액션: 외부망 Linux 컨테이너에서 `rr update --version v1.0.30` 후 daemon이 재시작된 상태로 `tail -n 120 ~/.local/state/rustory/daemon.log | rg 'dial timeout after|info: p2p|warn: p2p'`를 확인한다. retryable timeout은 `info:`이고, non-retryable 오류만 `warn:`이면 완료 처리한다.
