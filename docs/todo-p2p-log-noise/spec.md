...
- 완료: C1, C2, v1.0.30 릴리즈, 로컬 맥북 + k8s 5개 노드 배포. 외부망 Linux 컨테이너 v1.0.30 실측에서 일부 timeout은 `info`로 낮아졌지만, context가 한 문자열로 접힌 `p2p pull/push peer: ...: dial timeout after 12s` 형태가 아직 `warn`으로 남는 것을 확인해 v1.0.31 보완을 추가했다. v1.0.31은 로컬 맥북 + k8s 5개 노드에 배포했고 내부망 기준 tracker reachable 및 `pending_push=0`을 확인했다.
