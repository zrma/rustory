...
- Ctrl+r TUI polish: `rr search` now asks fzf for an inline height instead of fullscreen, hides fzf's count/separator noise, disables horizontal scroll for stable metadata columns, hides the pointer gutter, pads rows for legacy highlight behavior, enables full-row highlight only when the installed fzf supports it, and falls back to parsing transformed display rows from legacy fzf so ctrl+r still inserts only the command.
