...
- `RUSTORY_SEARCH_LIMIT=100000`: ctrl+r 검색 limit을 오버라이드한다.
  - unset이면 `config.toml`의 `search_limit_default`를 사용하고, config 값도 없으면 `100000`을 사용한다.
...
- 검색: `ctrl+r`에서 `rr search`(fzf)로 선택한 커맨드를 현재 입력 버퍼에 삽입한다. limit은 `RUSTORY_SEARCH_LIMIT` > `config.toml`의 `search_limit_default` > `100000` 순서로 해석된다.
