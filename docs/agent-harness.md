<!-- Generated by ai-first 1.7.3. Edit .ai-first.toml or .ai-first/overlays/, then render. -->
...
Rustory를 local-first 셸 히스토리 daily driver로 유지한다. 기록은 각 디바이스의
SQLite에 먼저 남고 tracker와 relay를 통한 P2P grid 동기화가 서로 다른 NAT 환경에서도
검증 가능해야 한다.
...
요청에 해당하는 단계를 수행한다. 설명·조사·리뷰·계획은 필요한 자료 확인과 재현·검증으로
답하며, 시작 절차만을 이유로 구현이나 전체 검사를 실행하지 않는다. 변경 작업은 기존
repository 필수 검증과 local closeout까지 수행한다.

1. 요청과 관련된 source of truth에서 시작한다. 변경·작업 재개·다음 과제 선택을 할
   때는 working copy, handoff/status와 관련 active packet으로 현재 상태를 복원하고,
   우선순위·후속 방향 판단에는 roadmap을 확인한다. 상세 문서는 repository 탐색 안내에
   따라 필요한 범위부터 읽되, 적용되는 agent 지침과 정체성·권한·공개 경계는 항상 준수한다.
...
4. repository-native lifecycle에 따라 milestone 또는 bounded specification의 todo
   ...
   boundary를 명시하고 누락·모순·미결정을 검토한다. 문제, 원하는 결과, 영향받는
   사용자·시스템과 유지해야 할 동작·제약을 구별한다. 기존 spec에 의도를 두고
   중복 intent 파일을 요구하지 않는다. 결과를 바꾸는 미결정은 의존 구현 전에
   해소하고, 완료 후 유지할 지식의 이관 위치를 정한다. 작은 변경은 native 규칙이
   허용하면 기존 issue/change 설명으로 충분하며 새 packet이나 PR을 강제하지 않는다.
5. 가장 작은 vertical slice를 구현하고 발견한 사실과 결정 변경을 spec에 반영한다.
   변경 전 기준을 VCS revision 또는 기존 versioned 기록으로 추적하고 변경 이유와
   영향을 남긴다. spec과 구현의 정합성을 유지하며 완료 기준 변경을 미충족 결과의
   은폐에 쓰지 않는다. 제품 방향·제약·위험의 실질적 변경에는 기존 escalation을 적용한다.
...
8. 변경 전 의도·제약·완료 기준과 이후 spec 변경을 diff 및 실제 결과·검증과 대조한다.
   문제 해결, 예상 밖 영향·범위 확장과 회귀 위험을 검토하고 미충족·미검증을 구별한다.
   유지할 결과, 결정 이유와 알려진 한계를 code/test, 사용법, architecture, decision
   record 또는 운영 문서 등 적절한 artifact에 반영한다. PR 또는 local change의
   리뷰 기록에는 짧은 의도·결과·검증 한계와 소유 artifact를 연결한다. 당시 spec이
   필요하면 immutable revision과 경로를 남기고 참조가 실제로 열리는지 확인한다.
   작업마다 별도 완료 보고서를 만들 필요는 없다.
...
- 리뷰 입력에는 변경 전 기준, 이후 spec 변경 이유, 대상 diff와 관련 검증을 포함한다.
  기준이 없거나 확인할 수 없으면 diff에서 원래 의도를 추측해 사실로 쓰지 않고
  누락과 한계를 표시한다. 구현의 자기평가나 PR의 체크 표시만으로 충족을 판정하지 않는다.
- 반복 리뷰의 immutable revision 또는 snapshot 지문과 당시 범위를 기록한다.
  중요한 요구사항·제약·영향 경로별 근거와 미확인 범위를 남기며, 지적 몇 개를 찾은
  것만으로 나머지 요청 범위를 검토했다고 보지 않는다.
- 기존 리뷰 기록에서 finding의 누적 상태를 유지한다. 후발 지적은 수정으로 생긴 회귀,
  이전 범위의 누락, 새 범위 또는 판정 불명을 구분한다. 이전 누락인지, 변경 전부터
  존재한 debt인지와 심각도는 별도 판단이며, 지적이 사라진 것만으로 해결 처리하지 않는다.
- 완료는 합의한 범위의 검토와 finding별 근거로 판단한다. 기록의 존재나 연속 무지적은
  완전성의 증거가 아니며 작은 변경에 별도 장부·고정 검토 횟수를 강제하지 않는다.
- 의도 항목의 제목과 필수 형식, work-start/readiness 명령은 repository-native 계층이
  소유한다. framework update는 기존 packet을 새 필수 필드로 거부하지 않는다.
- 리뷰 깊이와 문서량은 위험·복잡성에 비례한다. 추가 전문 리뷰나 다중 agent는
  확인된 필요와 허용 범위에서 선택하며, 리뷰 통과를 merge·publication 승인으로 해석하지 않는다.
