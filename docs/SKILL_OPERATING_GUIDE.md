...
2. 레포 아티팩트는 `사실/현황(What)`을 소유하되, 실행 동작은 코드/스크립트/매니페스트가 우선한다.
3. 전역 스킬에는 레포 가드(특정 레포명/경로)와 환경 고정값(엔드포인트/계정/리소스명)을 넣지 않는다.
4. 특정 레포에서만 유효한 규칙은 `docs/` 트리에서 네비게이션, 소유 경계, 결정 근거, 검증 증거 중심으로 관리한다.
...
2. 실제 경로/검증 명령의 선언 위치 (`docs/REPO_MANIFEST.yaml`, `scripts/*`, `cargo ...`)
3. 현재 배포 상태, 게이트 상태, 이행 TODO와 decision gate
4. 코드만으로 드러나지 않는 실사용 계약, 운영 제약, 결정 근거

실행 동작, 옵션 해석, default, 테스트 상세는 해당 코드/스크립트/CLI help를 직접 확인한다. 문서는 이를 반복 설명하지 않고 어디를 확인해야 하는지만 안내한다.
