...
- 에스컬레이션 조건에 해당하지 않는 작업은 에이전트가 저장소의 실행 루프와 게이트를 따라 자율적으로 진행한다.
- 진행 중 상태 공유는 하되, 세부 지시 요청으로 작업을 멈추지 않는다.
- 사람 호출은 아래 즉시 에스컬레이션 조건, 게이트 우회 승인, 또는 명시적 사용자 판단이 필요한 경우로 제한한다.
